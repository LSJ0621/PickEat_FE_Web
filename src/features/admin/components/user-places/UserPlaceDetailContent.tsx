      ...
      {isEditing && (
        <div>
          ...
          <UserPlaceImageUploader
            ...
            onExistingRemove={(url: string) => onEditFormChange({ ...editForm, existingPhotos: editForm.existingPhotos.filter((p) => p !== url) })}
            onNewAdd={(files: File[]) => onEditFormChange({ ...editForm, newImages: [...editForm.newImages, ...files] })}
            onNewRemove={(index: number) => onEditFormChange({ ...editForm, newImages: editForm.newImages.filter((_, i) => i !== index) })}
          />
        </div>
      )}
