...
export const MenuSelectionHistory = () => {
  ...
  const loadSelections = async () => {
    ...
    try {
      ...
      console.error('메뉴 선택 이력 조회 실패:', error);
      alert(extractErrorMessage(error, '메뉴 선택 이력을 불러오는데 실패했습니다.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error('메뉴 선택 수정 실패:', error);
  ...
  const handleCancel = async (selectionId: number) => {
    ...
    try {
      ...
      alert('메뉴 선택이 취소되었습니다.');
      await loadSelections();
    } catch (error: unknown) {
      console.error('메뉴 선택 취소 실패:', error);
      alert(extractErrorMessage(error, '메뉴 선택 취소에 실패했습니다.'));
    } finally {
      setIsUpdating(null);
    }
  };
