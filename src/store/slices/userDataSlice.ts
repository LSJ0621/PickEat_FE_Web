...
import type { UserAddress, Preferences } from '@/types/user';
import { userService } from '@/api/services/user';
import { CACHE_STALE_MS } from '@/utils/constants';
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAddresses.rejected, (state, action) => {
        state.addresses.isLoading = false;
        state.addresses.isDirty = true;
        console.error('Failed to fetch addresses:', action.error.message);
      })
      ...
      .addCase(fetchPreferences.rejected, (state, action) => {
        state.preferences.isLoading = false;
        state.preferences.isDirty = true;
        console.error('Failed to fetch preferences:', action.error.message);
      })
