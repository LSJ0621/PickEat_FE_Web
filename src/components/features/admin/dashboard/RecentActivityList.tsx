...
export function RecentActivityList({ activities }: RecentActivityListProps) {
  ...
  return (
    <Card className="bg-bg-surface border-[var(--border-default)]">
      <CardHeader>
        <CardTitle className="text-lg font-semibold text-text-primary">최근 활동</CardTitle>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="users" className="w-full">
          <TabsList className="grid w-full grid-cols-3 bg-bg-surface">
            <TabsTrigger value="users" className="data-[state=active]:bg-bg-secondary">
              <UserCheck className="h-4 w-4 mr-2" />
              최근 가입자
            </TabsTrigger>
            <TabsTrigger value="bugReports" className="data-[state=active]:bg-bg-secondary">
              <FileText className="h-4 w-4 mr-2" />
              버그 리포트
            </TabsTrigger>
            <TabsTrigger value="deletedUsers" className="data-[state=active]:bg-bg-secondary">
            ...
            <div className="space-y-3">
              {activities.recentUsers.length === 0 ? (
                ...
                activities.recentUsers.map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between p-3 bg-bg-surface rounded-lg hover:bg-bg-hover transition-colors"
                  >
                    ...
                    <span className="text-sm text-text-tertiary">{formatDate(user.createdAt)}</span>
                  </div>
                ))
              )}
            </div>
              ...
              {activities.recentBugReports.length === 0 ? (
                ...
                activities.recentBugReports.map((report) => (
                  <button
                    ...
                    className="w-full flex items-center justify-between p-3 bg-bg-surface rounded-lg hover:bg-bg-hover transition-colors text-left"
                  >
                    <div className="flex-1">
                    ...
                    <span className="text-sm text-text-tertiary">{formatDate(report.createdAt)}</span>
                  </button>
                ))
              )}
            </div>
              ...
              {activities.recentDeletedUsers.length === 0 ? (
                ...
                activities.recentDeletedUsers.map((user) => (
                  ...
                  >
                    ...
                    <span className="text-sm text-text-tertiary">{formatDate(user.deletedAt)}</span>
                  </div>
                ))
              )}
            </div>
