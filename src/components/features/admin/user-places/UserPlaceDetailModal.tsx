  ...
  const modalContent = (
    <>
      ...
      >
        ...
        <div
          ref={modalContentRef}
          className={`relative z-10 w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-lg border border-slate-700 bg-slate-900 p-6 shadow-xl custom-scroll ${
        ...
        >
          ...
          <div className="mb-6 flex items-start justify-between">
            <div className="flex-1">
              <h2 className="mb-2 text-2xl font-bold text-white">{place.name}</h2>
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-3 rounded-lg border border-slate-700 bg-slate-800/50 p-4">
              <div>
                ...
                {isEditing ? (
                  <input
                    ...
                    className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-white focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                  />
                ) : (
              ...
              <div>
                <div className="text-sm text-slate-400">주소</div>
                {isEditing ? (
                  <input
                    type="text"
              ...
              <div>
                ...
                {isEditing ? (
                  ...
                  <div className="text-white">{place.category}</div>
                )}
              </div>
              <div>
                <div className="text-sm text-slate-400">전화번호</div>
                {isEditing ? (
                  <input
                    type="text"
                  ...
                  <div className="text-white">{place.phoneNumber || '-'}</div>
                )}
              </div>
              <div>
                <div className="text-sm text-slate-400">영업시간</div>
                {isEditing ? (
                  <input
                    type="text"
                  ...
                  <div className="text-white">{place.openingHours || '-'}</div>
                )}
              </div>
              <div>
                <div className="text-sm text-slate-400">설명</div>
                {isEditing ? (
                  <textarea
                  ...
                  <div className="text-white">{place.description || '-'}</div>
                )}
              </div>
              {isEditing && (
                <div>
                  <div className="text-sm text-slate-400 mb-2">사진</div>
              ...
              <div>
                <div className="text-sm text-slate-400">등록자</div>
                <div className="text-white">{place.user.email}</div>
              </div>
            </div>
            ...
            <div className="space-y-3 rounded-lg border border-slate-700 bg-slate-800/50 p-4">
              <div>
                <div className="text-sm text-slate-400">등록일</div>
                <div className="text-white">{formatDate(place.createdAt)}</div>
              </div>
              <div>
                <div className="text-sm text-slate-400">수정일</div>
                <div className="text-white">{formatDate(place.updatedAt)}</div>
              </div>
              {place.rejectionCount > 0 && (
                <div>
                  ...
                  <div className="text-orange-400">{place.rejectionCount}회</div>
                </div>
              )}
              {place.lastRejectedAt && (
                <div>
                  <div className="text-sm text-slate-400">마지막 거절 일시</div>
                  <div className="text-white">{formatDate(place.lastRejectedAt)}</div>
                </div>
              )}
              {place.rejectionReason && (
                <div>
                  <div className="text-sm text-slate-400">거절 사유</div>
                  <div className="text-white">{place.rejectionReason}</div>
                </div>
              )}
            </div>
            ...
            {isEditing && (
              <div className="flex gap-3">
                <button
                  ...
                  disabled={isUpdating}
                  className="flex-1 rounded-lg border border-slate-700 bg-slate-800 px-4 py-3 font-medium text-white transition hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Cancel
                </button>
                <button
                  ...
                  disabled={isUpdating}
                  className="flex-1 rounded-lg bg-orange-500 px-4 py-3 font-medium text-white transition hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
            ...
            {place.status === 'PENDING' && !isEditing && (
              <div className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-4">
                    <label htmlFor="action-approve" className="flex items-center gap-2 cursor-pointer">
                      ...
                      />
                      <span className="text-white">{t('admin.userPlaces.detail.approve')}</span>
                    </label>
                    <label htmlFor="action-reject" className="flex items-center gap-2 cursor-pointer">
                      <input
                        ...
                        className="h-4 w-4 text-orange-500 focus:ring-orange-500"
                      />
                      <span className="text-white">{t('admin.userPlaces.detail.reject')}</span>
                    </label>
                  </div>
                </div>
                ...
                {selectedAction === 'reject' && (
                  <div className="rounded-lg border border-slate-700 bg-slate-800 p-4 space-y-3">
                    ...
                    <textarea
                      ...
                      className="w-full rounded-lg border border-slate-700 bg-slate-900 px-4 py-3 text-white placeholder-slate-500 focus:border-orange-500 focus:outline-none focus:ring-2 focus:ring-orange-500/20"
                      rows={4}
                      maxLength={500}
                    />
                    {rejectError && (
                      <p className="text-sm text-red-400">{rejectError}</p>
                    )}
                ...
                <button
                  ...
                  className="w-full rounded-lg bg-orange-500 px-4 py-3 font-medium text-white transition hover:bg-orange-600 disabled:opacity-50 disabled:cursor-not-allowed"
                >
