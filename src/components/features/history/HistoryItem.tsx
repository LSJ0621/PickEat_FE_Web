...
export const HistoryItem = ({ item }: HistoryItemProps) => {
  ...
  const hasAiQueryContext = Boolean(item.requestAddress?.trim() || address?.trim() || hasLocation);
  ...
  const handleSearchWithClose = async () => {
    ...
    try { await menuActions.handleSearch(); } catch { /* handled internally */ }
  };
      ...
      <div className="rounded-2xl border border-border-default bg-bg-surface p-5 shadow-md transition-shadow hover:shadow-lg">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {item.recommendations.map((menu, index) => (
            <button key={index} onClick={() => handleMenuClickWithReset(menu)}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm"
          ...
          <div className="relative w-full max-w-md rounded-2xl border border-border-default bg-bg-surface p-8 shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Button onClick={handleSearchWithClose} disabled={!hasLocation} variant="primary" size="lg">{t('history.normalSearch')}</Button>
              <Button onClick={handleAiRecommendWithClose} variant="secondary" size="lg" disabled={!hasAiQueryContext}>{t('history.aiRecommendGet')}</Button>
            </div>
            {!hasLocation && <p className="mt-4 rounded-xl border border-amber-300/30 bg-amber-500/10 p-3 text-center text-sm text-amber-200">{t('history.noLocationWarning')}</p>}
            {!hasAiQueryContext && <p className="mt-2 rounded-xl border border-amber-300/30 bg-amber-500/10 p-3 text-center text-sm text-amber-200">{t('history.needsAddressForAi')}</p>}
          </div>
        </div>
      )}
      ...
      {!menuActions.showConfirmCard && menuActions.selectedMenu && (
        <div className="mt-4 space-y-4">
          {menuActions.restaurants.length > 0 && (
            <RestaurantList menuName={menuActions.selectedMenu} restaurants={menuActions.restaurants} loading={menuActions.loading} onClose={menuActions.resetSearchResults} />
          )}
